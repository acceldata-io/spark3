...
class HiveTempPath(session: SparkSession, val hadoopConf: Configuration, path: Path)
  ...
  private def getExternalTmpPath(path: Path): Path = {
    ...
    // Hive creates the staging directory under the table directory, and when moving the staging
    // directory to the table directory, Hive will still empty the table directory but will
    // exclude the staging directory there.
